...
def generate_launch_description():
    ...
    hunav_loader_node = Node(
        ...
        output="screen",
        parameters=[agent_conf_file, {"use_sim_time": use_sim_time}],
    ...
    ordered_launch_event2 = RegisterEventHandler(
        OnProcessStart(
            ...
            on_start=[
                LogInfo(
                    msg="GenerateWorld started, launching Gazebo after 2 s..."
                ),
                TimerAction(
                    period=2.0,
                    actions=[
        ...
        parameters=[
            metrics_file,
            {
                "result_file": os.path.join(
                    hunav_pkg_dir, "evaluation", "eval", "metrics.txt"
                )
            },
            {"use_sim_time": use_sim_time},
        ],
    ...
    declare_agents_conf_file = DeclareLaunchArgument(
        "configuration_file",
        default_value="agents_eng_hall.yaml",
    ...
    declare_metrics_conf_file = DeclareLaunchArgument(
        ...
        description=(
            "Specify the name of the metrics configuration file in the config "
            "directory"
        ),
    )
    ...
    declare_frame_to_publish = DeclareLaunchArgument(
        ...
        default_value="map",
        description=(
            "Name of the global frame in which the position of the agents "
            "are provided"
        ),
    )
    declare_use_navgoal = DeclareLaunchArgument(
        ...
        default_value="false",
        description=(
            "Whether to start the agents movements when a navigation goal is "
            "received or not"
        ),
    )
    declare_insert_collision_cylinder = DeclareLaunchArgument(
        "insert_collision_cylinder",
        default_value="false",
        description="Whether to add a collision cylinder for the actors",
    )
    declare_ignore_models = DeclareLaunchArgument(
        "ignore_models",
        default_value="ground_plane cafe",
        description=(
            "list of Gazebo models that the agents should ignore as "
            "obstacles as the ground_plane. Indicate the models with a "
